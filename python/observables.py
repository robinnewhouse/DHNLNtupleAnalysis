...
def queue_all_observables(analysis_name): 
	...
	for i in range(len(MCEventType)):
		...
		truth_histograms("all_truth")
		truth_histograms("LNC_truth")
		truth_histograms("LNV_truth")
		truth_histograms("LNC_truth_presel")
		truth_histograms("LNC_truth_mass")
		truth_histograms("LNC_truth_charge")
		truth_histograms("LNC_truth_DVtype")
		truth_histograms("LNC_truth_trkqual")
		truth_histograms("LNC_truth_cosmic")
		truth_histograms("LNC_truth_mlll")
		truth_histograms("LNC_truth_mDV")
		truth_histograms("LNC_truth_sel")
		truth_histograms("LNC_truth_match")
		truth_histograms("LNV_truth_presel")
		truth_histograms("LNV_truth_mass")
		truth_histograms("LNV_truth_charge")
		truth_histograms("LNV_truth_DVtype")
		truth_histograms("LNV_truth_trkqual")
		truth_histograms("LNV_truth_cosmic")
		truth_histograms("LNV_truth_mlll")
		truth_histograms("LNV_truth_mDV")
		truth_histograms("LNV_truth_sel")
		truth_histograms("LNV_truth_match")
...
def reco_histograms(selection):
	...
	Observable(selection + "_nonlep_pt", binning=(1000, 0, 1000), do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_nonlep_eta", binning=(40, -10, 10), do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_nonlep_phi", binning=(16, -4, 4), do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_nonlep_d0", binning=(2000, -1000, 1000), do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_plep_pt", binning=(1000, 0, 1000), do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_plep_eta", binning=(40, -10, 10), do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_plep_phi", binning=(16, -4, 4), do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_plep_d0", binning=(2000, -1000, 1000), do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_plep_z0", binning=(2000, -1000, 1000), do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_plep_charge", binning=(3,-1.5,1.5), do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_weight", binning=(1, -10, 10), do=['oldAnalysis','ToyAnalysis','KShort']).queue()  # dummy histogram
	Observable(selection + "_DV_x",binning = (2000,-500,500), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_y",binning = (2000,-500,500), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_z",binning = (2000,-500,500), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_r",binning = (500,0,500), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_num_trks",binning = (6,-0.5,5.5), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_presel_num_trks",binning = (21,-0.5,20.5), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_distFromPV",binning = (500,0,500), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_mass", binning=(2000, 0, 1000), do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_pt",binning = (1000,0,1000), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_sum_track_pt",binning = (1000,0,1000), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_sum_track_pt_wrt_pv",binning = (1000,0,1000), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_sum_track_pt_diff",binning = (10000,0,100), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_sum_track_charge",binning = (4,-1.5,1.5), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_eta",binning = (160,-10,10),do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_phi", binning = (64,-4,4), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_charge",binning = (11,-5.5,5.5), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_chi2",binning = (600,0,30), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_chi2_assoc",binning = (600,0,30), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_alpha",binning = (400,-4,4), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_mvis",binning = (10000,0,5000), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_HNLm",binning = (10005,-5,5000), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_HNLm_altbinning",binning = (15,0,30), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_alt_HNLm",binning = (10005,-5,5000), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_HNLm_fixWmass",binning = (10005,-5,5000), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_HNLpt",binning = (1000,0,1000), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_HNLeta",binning = (160,-10,10),do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_HNLphi", binning = (64,-4,4), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_mtrans",binning = (10000,0,5000), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_redmass",binning = (10005,-5,5000), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_redmassvis",binning = (10005,-5,5000), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_redmassHNL",binning = (10005,-5,5000), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_2tight",binning = (2,-0.5,1.5), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_2medium",binning = (2,-0.5,1.5), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_2loose",binning = (2,-0.5,1.5), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_1tight",binning = (2,-0.5,1.5), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_1medium",binning = (2,-0.5,1.5), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_1loose",binning = (2,-0.5,1.5), do = ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_trk_max_chi2_toSV", binning = (600,0,30), do= ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_trk_min_chi2_toSV", binning = (600,0,30), do= ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_trk_max_d0_wrtSV", binning = (1000,-50,50), do= ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_trk_min_d0_wrtSV", binning = (1000,-50,50), do= ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_trk_max_errd0_wrtSV", binning = (10000,0,50), do= ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_trk_min_errd0_wrtSV", binning = (10000,0,50), do= ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_trk_max_z0_wrtSV", binning = (3000,-30,30), do= ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_trk_min_z0_wrtSV", binning = (3000,-30,30), do= ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_trk_max_errz0_wrtSV", binning = (10000,0,50), do= ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_trk_min_errz0_wrtSV", binning = (10000,0,50), do= ['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_max_dR", binning = (1000,0,10),do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_max_dR_wrtSV", binning = (1000,0,10),do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_maxOpAng", binning = (2000,-1,1),do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_minOpAng", binning = (2000,-1,1),do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_maxd0", binning = (1000,-250,250),do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_mind0", binning = (1000,-250,250),do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_ntrk", binning = (3,-0.5,2.5),do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_ntrk_lrt", binning = (3,-0.5,2.5),do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_ntrk_sel", binning = (3,-0.5,2.5),do=['oldAnalysis','ToyAnalysis','KShort']).queue()
	Observable(selection + "_DV_ntrk_assoc", binning = (3,-0.5,2.5),do=['oldAnalysis','ToyAnalysis','KShort']).queue()
