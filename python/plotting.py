...
def plot_cutflow(file, vertextype, outputDir="../output/"):
    ...
    if "uuu" in file:
        ...
        helpers.drawNote(vertextype, size=50, ax=0.7)
...
def compare(hist_channels, variable="", setrange=None, scaleymax=1.2, nRebin=1, setlogy=False, outputDir="../output/",
    ...
    for key, val in hist_channels.items():
        ...
        histname = "{}_{}_{}".format(val[2], variable, val[1])
    ...
    for i in h_idx:
        ...
        histograms[i].SetLineColor(helpers.histColours(i))
        histograms[i].SetLineWidth(3)
        histograms[i].SetMarkerColor(helpers.histColours(i))
        histograms[i].SetMarkerStyle(shapelist[i])
        histograms[i].GetXaxis().SetTitle(helpers.get_x_label(variable))
    ...
    # helpers.getNote(35).DrawLatex(notes_x, notes_y-.05, vertextype)
...
def compare_dataMC(datafile, mcfiles, hname, hdatalabel, hmclabels, vertextype, setrange=None,
    ...
    for i in xrange(nmc_files):
        ...
        hmc[i].GetXaxis().SetTitle(helpers.xlabelhistograms(hname))
