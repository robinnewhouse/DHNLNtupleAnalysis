...
class HNLMassCalculation:
    ...
    Inputs:
    - pv: ROOT TVector3 object for the location (x,y,z) of the primary vertx (PV).
    - dv: ROOT TVector3 object for the location (x,y,z) of the displaced vertx (DV).
    - displaced_leptons: Python list of ROOT TVector3 objects for the momenta (px,py, pz) of the two leptons in the DV. Units: GeV.
    - prompt_lepton: ROOT TVector3 object for the momentum (px, py, pz) of the prompt lepton. Units: GeV.
    ...
    For details about ROOT TVector3 objects see: https://root.cern.ch/doc/master/classTVector3.html
    """
    ...
    def __init__(self, pv, dv, displaced_leptons, prompt_lepton, fix_w_mass=False):
        ...
        p0 = ROOT.TVector3(prompt_lepton.X(), prompt_lepton.Y(), prompt_lepton.Z())

        # Assign d0 and d1 to be the 3-vectors for the displaced leptons
        d0 = ROOT.TVector3(displaced_leptons[0].X(), displaced_leptons[0].Y(), displaced_leptons[0].Z())
        d1 = ROOT.TVector3(displaced_leptons[1].X(), displaced_leptons[1].Y(), displaced_leptons[1].Z())
