...
class InvertedPromptLepton():
	def __init__(self, evt, d0_cut=3.0, z0_sin_theta_cut=0.5, pt_cut=25.0):
		...
		for imu in range(n_muons): 
			...
			if abs(mud0) < d0_cut and abs(muz0sintheta) < z0_sin_theta_cut:
				self.Pmuons_4vec.append(muVec_i)
				self.Pmuons_d0.append(mud0)
				self.n_prompt_muons += 1 # count the number of prompt leptons 
			else:
				self.nonPmuons_4vec.append(muVec_i)
				self.nonPmuons_d0.append(mud0)
		...
		for iel in range(n_electrons): 
			...
			if abs(eld0) < d0_cut and abs(elz0sintheta) < z0_sin_theta_cut:
