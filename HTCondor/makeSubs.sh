...
DirForSubs=/nfs/dust/atlas/user/appelt/R22/data_SS_2018_eee_VSI_Lep_Mod_3
...
for eachfile in "$dataDirectory"/* #loop over files in data directory
 do
   ...
   echo "cd ../python" >> "$file"
   echo "python makeHistograms.py -i \$inputFile --config ../data/SSbkg/config_SSbkg_eee.json --output_file ../output/output.root --useNTAU -f"  >> "$file"
